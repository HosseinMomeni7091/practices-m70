...
<div class="p-4 mb-4 text-sm text-blue-700 bg-blue-100 rounded-lg dark:bg-blue-200 dark:text-blue-800" role="alert">
    <span class="font-medium">Restaurant Categories</span>
</div>
...
<div class=" px-4 py-2 mb-4 text-sm w-1/2 mx-auto text-blue-300 bg-blue-100 rounded-lg dark:bg-blue-200 dark:text-blue-800" role="alert">
    <form class="flex  items-center" action="{{route('restcategory.store')}}" method="POST">
        @csrf
        ...
        <tbody>
            ...
            <tr class="border-b dark:bg-gray-800 dark:border-gray-700 odd:bg-amber-100 even:bg-gray-200 odd:dark:bg-gray-800 even:dark:bg-gray-700">
                ...
                <td class="px-6 py-4 text-right">
                    <form action="{{ route('restcategory.update',['restcategory'=>$restcategory]) }}" method="POST" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                        @method('PUT')
                        @CSRF
                        <input type="hidden" name="editeid" value="{{$restcategory->id}}">
                ...
                <td class="px-6 py-4 text-right">
                    <form action="{{ route('restcategory.destroy',['restcategory'=>$restcategory]) }}" method="POST" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                        @method('DELETE')
                        @CSRF
                        <input type="hidden" name="remove" value="{{$restcategory->id}}">
                        <button class=" mx-auto bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Remove
                        </button>
                    </form>
